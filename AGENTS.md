...
- **Never add bold weights to `font-display` headings:** `font-display` (Bebas Neue) naturally carries strong optical weight. Never apply `font-bold`, `font-semibold`, or `<strong>` tags to `font-display` text (e.g. use `font-display text-base tracking-wide ...`, not `font-bold font-display`).
- **Never use `<span>` for Bebas Neue (`font-display`):** Bebas Neue represents semantic headlines, section titles, and card headers. Always use semantic heading elements (`<h1>` through `<h6>`) or semantic block elements (`<p>`). Never use inline `<span>` tags with `font-display`.
- **PageHeader standard:** `<PageHeader title="..." actions={...} />` — **never add `description` or subtitle copy to `PageHeader`**. Keep page headers clean with only title and top-level action buttons.
- **Dynamic Enums via Hooks:** Never hardcode enum values, arrays, or label dictionaries in `app/types/*.ts` or features. Always consume the dedicated hooks: `useClassModes()`, `useDays()`, `useSessionModes()`, and `useEnums()` (which query `GET /enums`).
