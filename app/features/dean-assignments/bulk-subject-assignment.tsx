...
function Summary({ icon, label, value }: { icon: ReactNode; label: ReactNode; value: ReactNode }) { return <div className="flex min-w-32 items-center gap-3 border-l border-slate-200 pl-5 dark:border-white/10"><span className="text-navy-700 dark:text-gold-300">{icon}</span><div><p className="font-body text-xs text-slate-500 dark:text-slate-400">{label}</p><div className="mt-1 font-body text-base font-bold text-navy-800 dark:text-white">{value}</div></div></div>; }
