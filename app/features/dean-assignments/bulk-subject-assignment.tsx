...
import { BookOpenIcon, ClockIcon, CloseIcon, LayersIcon, SearchIcon } from "~/components/ui/icons";
import { formatInstructorName } from "~/lib/faculty-load";
import type { DepartmentInstructor } from "~/services/dean.service";
import type { useDeanSubjectAssignments } from "~/features/dean-assignments/use-dean-subject-assignments";
...
function instructorInitials(i: DepartmentInstructor) {
  return i.lastName[0]?.toUpperCase() ?? "";
}

function Summary({ icon, label, value }: { icon: ReactNode; label: ReactNode; value: ReactNode }) {
  return (
    <div className="flex min-w-32 items-center gap-3 border-l border-slate-200 pl-5 dark:border-white/10">
      ...
      <div>
        ...
        <div className="mt-1 font-body text-base font-bold text-navy-800 dark:text-white">{value}</div>
      </div>
    </div>
  );
}

type BulkAssignPanelProps = {
  data: ReturnType<typeof useDeanSubjectAssignments>;
  onClose: () => void;
};

export function BulkAssignPanel({ data, onClose }: BulkAssignPanelProps) {
  const [states, setStates] = useState<Record<string, InstructorState>>({});
  ...
  const stateFor = (instructor: DepartmentInstructor) =>
    states[keyFor(instructor)] ?? { hours: "", selected: new Set<string>() };
  const update = (instructor: DepartmentInstructor, fn: (value: InstructorState) => InstructorState) =>
    setStates((all) => ({
      ...all,
      [keyFor(instructor)]: fn(all[keyFor(instructor)] ?? { hours: "", selected: new Set<string>() }),
    }));
  ...
  async function save() {
    ...
    if (instructorLoads.length) {
      await data.createAssignments(instructorLoads);
    }
    onClose();
  }

  return (
    <div className="mt-4 rounded-xl border border-slate-300 bg-white p-5 dark:border-white/10 dark:bg-white/5">
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 font-body text-sm text-slate-500 dark:text-slate-400">
            Select subjects for each instructor and set their maximum weekly hours.
          </p>
        </div>
        <button
          type="button"
          onClick={onClose}
          className="grid size-8 place-items-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100 hover:text-navy-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400 dark:hover:bg-white/10 dark:hover:text-white"
          aria-label="Close bulk assign panel"
        >
          <CloseIcon />
        </button>
      </div>
      ...
      <Accordion>
        {instructors.map((instructor) => {
          const key = keyFor(instructor);
          const state = stateFor(instructor);
          const programs = new Set([...state.selected].map((item) => item.split(":")[0])).size;

          return (
            <AccordionItem
              key={key}
              title={
                <div className="grid w-full gap-5 lg:grid-cols-[1.45fr_repeat(3,.8fr)] lg:items-center">
                    ...
                    value={
                      <div className="flex w-28 items-center">
                        <input
                          ...
                          onChange={(event) =>
                            update(instructor, (current) => ({ ...current, hours: event.target.value }))
                          }
                          className={`${inputClassName} rounded-r-none py-1.5 font-semibold`}
                        />
                        <span className="rounded-r-lg border border-l-0 border-slate-300 bg-slate-50 px-2 py-1.5 font-body text-sm dark:border-white/15 dark:bg-white/10">
                          hrs
                        </span>
                      </div>
                    }
                  />
                  <Summary icon={<LayersIcon />} label="Assigned Programs" value={programs} />
                  <Summary icon={<BookOpenIcon />} label="Selected Subjects" value={state.selected.size} />
                </div>
              }
            >
              <div
                className={`grid gap-4 bg-slate-50/60 p-4 ${(data.subjects ?? []).length <= 1 ? "grid-cols-1" : "xl:grid-cols-2"} dark:bg-navy-950/30`}
              >
                {(data.subjects ?? []).map((program) => {
                  const prefix = program.programAbbrev || program.programName;
                  const allSubjects = program.curriculumDetails.flatMap((year) =>
                    year.semesterDetails.flatMap((semester) => semester.subjects),
                  );
                  const sq = subjectSearch.toLowerCase();
                  const subjects = sq
                    ? allSubjects.filter(
                        (subject) =>
                          subject.subjectCode.toLowerCase().includes(sq) ||
                          subject.descriptiveTitle.toLowerCase().includes(sq),
                      )
                    : allSubjects;
                  const count = allSubjects.filter((subject) =>
                    state.selected.has(`${prefix}:${subject.subjectCode}`),
                  ).length;
                    ...
                    >
                      <header className="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 dark:border-white/10">
                        <div>
                          ...
                          <p className="mt-1 font-body text-xs text-slate-500 dark:text-slate-400">{prefix}</p>
                        </div>
                        <span className="rounded-full bg-navy-100 px-2.5 py-1 font-body text-xs font-semibold text-navy-800 dark:bg-white/10 dark:text-white">
                          {count} / {allSubjects.length} selected
                        </span>
                      </header>
                      <div className="border-t border-slate-100 px-4 py-2 dark:border-white/10">
                        <div className="relative">
                          ...
                          <input
                            ...
                            placeholder={`Search ${prefix} subjects\u2026`}
                      ...
                      <div className="max-h-80 overflow-y-auto">
                        {subjects.length === 0 ? (
                          <p className="px-4 py-6 text-center font-body text-sm text-slate-400">
                            No matching subjects.
                          </p>
                        ) : (
                          subjects.map((subject) => {
                            const subjectKey = `${prefix}:${subject.subjectCode}`;
                            return (
                              <div
                                key={subject.subjectCode}
                                className="grid grid-cols-[auto_7rem_1fr_auto] items-center gap-3 border-b border-slate-100 px-4 py-3 last:border-0 dark:border-white/10"
                              >
                                <Checkbox
                                  id={`bulk-${key}-${subjectKey}`}
                                  label=""
                                  ariaLabel={`Assign ${subject.subjectCode}`}
                                  checked={state.selected.has(subjectKey)}
                                  onChange={(checked) =>
                                    update(instructor, (current) => {
                                      const selected = new Set(current.selected);
                                      checked ? selected.add(subjectKey) : selected.delete(subjectKey);
