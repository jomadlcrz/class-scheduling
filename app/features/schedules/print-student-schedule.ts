  ...
  return `
    <section class="sp-day">
      ...
      <table class="${showSet ? "sp-with-set" : ""}">
        <thead>
          <tr><th>TIME</th><th>SUBJECT CODE</th><th>DESCRIPTIVE TITLE</th>${setTh}<th>MODE</th><th>INSTRUCTOR</th><th>ROOM</th></tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
    </section>
...
/**
 * Open a student's class schedule as a printable document in a new tab —
 * includes the student's name in the header and optionally shows the Set
 * column for irregular students.
 */
export function openStudentSchedulePrint(
  schedules: Schedule[],
  context: { schoolYear: string; semesterLabel: string; studentName: string; showSet?: boolean },
): boolean {
  if (schedules.length === 0) return false;

  const origin = window.location.origin;
  const { departmentCode } = schedules[0];
  const showSet = context.showSet ?? false;
  ...
  const dayBlocks = dayGroups.map(({ day, slots }) => renderDayGroup(day, slots, showSet)).join("");
...
<body>
  <header class="sp-header">
    ...
    <h2>${safe(context.studentName)}</h2>
    ${!showSet && schedules[0] ? `<p>${safe(schedules[0].setCode)}</p>` : ""}
