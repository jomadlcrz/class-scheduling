...
const linkClassName = ({ isActive }: { isActive: boolean }) =>
  `flex shrink-0 items-center gap-2 rounded-lg px-3 py-2 font-body text-sm whitespace-nowrap transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400 ${
    isActive
...
export default function SettingsLayout() {
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-6 px-4 py-8">
      <nav aria-label="Settings" className="flex flex-row gap-1 overflow-x-auto scrollbar-none">
        {visibleNav.map((item) => (
